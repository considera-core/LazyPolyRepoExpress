:: FnTerraformDispatch --suite <SUITE> --action <ACTION> [--project <PROJECT>]
::               [-p <PROJECTS...>] [-a <ARGS...>] [FLAGS...]
:: -- Validates the action for the terraform module and invokes the leaf function
:: --
:: --   FnTerraform<Action> SUITE PROJECT ARGS... FLAGS...          (single project)
:: --   FnTerraform<Action> SUITE ARGS... -p PROJECTS... FLAGS...   (selected projects)
:: --   FnTerraform<Action> SUITE ARGS... FLAGS...                  (all projects)
:: --
...
SET "M_MODULE=Terraform"
SET "M_LABEL=terraform"
:: <action>:<function suffix>
SET "M_ACTIONS=init:Init plan:Plan apply:Apply"
...
:MISSING_SUITE
    ECHO LeprechaunCLI:FnTerraformDispatch[E]: Missing required flag --suite ^<SUITE^>
    GOTO :USAGE

:MISSING_ACTION
    ECHO LeprechaunCLI:FnTerraformDispatch[E]: Missing action
    GOTO :USAGE

:UNKNOWN_ACTION
    ECHO LeprechaunCLI:FnTerraformDispatch[E]: Unknown terraform action "%M_ACTION%"
    GOTO :USAGE
