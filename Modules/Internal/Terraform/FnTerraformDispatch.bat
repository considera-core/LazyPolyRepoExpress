:: FnTerraformDispatch <SuiteId> <ProjectId> <ActionId> <Arg[]> <Flag[]>
:: leprechaun function terraform dispatch <SuiteId> <ProjectId> <ActionId> <Arg[]> <Flag[]>
:: -- Validates the action for the terraform module and invokes the leaf function:
:: --
:: --   FnTerraform<Action> SuiteId ProjectId --args ARGS... FLAGS...   (single project)
:: --   FnTerraform<Action> SuiteId --projects P... --args ARGS...      (selected projects)
:: --   FnTerraform<Action> SuiteId --args ARGS... FLAGS...             (all projects)
:: --
...
:Missing
    CALL FnEtcLogRun FnTerraformDispatch "fn=Fn%Function_Pascal%%Function_Suffix% suite=%Function_SuiteId% project=%Function_ProjectId% args=%Function_Args% projects=%Function_Projects% flags=%Function_Passthru%"
    IF DEFINED Function_DryRun GOTO Destructor
    SET "Function_Error=Not implemented yet: Fn%Function_Pascal%%Function_Suffix%"
    GOTO Failure

:Constructor
    SET "Function_Module=terraform"
    SET "Function_Pascal=Terraform"
    :: <action>:<function suffix>
    SET "Function_Actions=init:Init plan:Plan apply:Apply"
...
:Usage
    CALL FnEtcLogError FnTerraformDispatch "%Function_Error%"
    SET "Function_Error="
    ECHO   Usage: ^<SUITE^> terraform ^<ACTION^> ^<ARGS...^> ^<FLAGS...^>
    ECHO          ^<SUITE^> ^<PROJECT^> terraform ^<ACTION^> ^<ARGS...^>
    ECHO          ^<SUITE^> terraform ^<ACTION^> -p ^<PROJECTS...^>
...
:Destructor
    ...
    IF DEFINED Function_Error CALL FnEtcLogError FnTerraformDispatch "%Function_Error%"
