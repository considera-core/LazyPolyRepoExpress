...
:: leprechaun function ai dispatch <SuiteId> <ProjectId> <ActionId> <Arg[]> <Flag[]>
:: -- Validates the action for the ai module and invokes the leaf function:
:: --
:: --   FnAi<Action> SuiteId ProjectId --args ARGS... FLAGS...   (single project)
:: --   FnAi<Action> SuiteId --projects P... --args ARGS...      (selected projects)
:: --   FnAi<Action> SuiteId --args ARGS... FLAGS...             (all projects)
:: --
...
:Missing
    CALL FnEtcLogRun FnAiDispatch "fn=Fn%Function_Pascal%%Function_Suffix% suite=%Function_SuiteId% project=%Function_ProjectId% args=%Function_Args% projects=%Function_Projects% flags=%Function_Passthru%"
    IF DEFINED Function_DryRun GOTO Destructor
    SET "Function_Error=Not implemented yet: Fn%Function_Pascal%%Function_Suffix%"
    GOTO Failure

:Constructor
    SET "Function_Module=ai"
    SET "Function_Pascal=Ai"
    :: <action>:<function suffix>
    SET "Function_Actions=claude:Claude"
...
:Usage
    CALL FnEtcLogError FnAiDispatch "%Function_Error%"
    SET "Function_Error="
    ECHO   Usage: ^<SUITE^> ai ^<ACTION^> ^<ARGS...^> ^<FLAGS...^>
    ECHO          ^<SUITE^> ^<PROJECT^> ai ^<ACTION^> ^<ARGS...^>
    ECHO          ^<SUITE^> ai ^<ACTION^> -p ^<PROJECTS...^>
    ECHO   Actions: claude
    GOTO Failure
...
:Destructor
    ...
    IF DEFINED Function_Error CALL FnEtcLogError FnAiDispatch "%Function_Error%"
