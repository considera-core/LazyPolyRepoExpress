...
FOR %%T IN (%*) DO (
    ...
    IF NOT DEFINED Local_Mode (
        ...
        IF DEFINED DEBUG CALL FnEtcLogDebug FnEtcFlags2 "--- [ARG] SET Local_AllArgs: !Local_AllArgs!"
    )
)
...
:: Local_Args becomes the list of arguments that are not known parameters (basically remove known parameters from Local_AllArgs)
SET "Local_Args=!Local_AllArgs!"
FOR %%K IN (!Local_KnownParameters!) DO SET "Local_Args=!Local_Args: %%K=!"
...
IF DEFINED DEBUG CALL FnEtcLogDebug FnEtcFlags2 "SET Output_FnEtcFlags_Args: %Output_FnEtcFlags_Args%"
EXIT /B 0
