:: FnIdeDispatch <SuiteId> <ProjectId> <ActionId> <Arg[]> <Flag[]>
:: leprechaun function ide dispatch <SuiteId> <ProjectId> <ActionId> <Arg[]> <Flag[]>
:: -- Validates the action for the ide module and invokes the leaf function:
:: --
:: --   FnIde<Action> SuiteId ProjectId --args ARGS... FLAGS...   (single project)
:: --   FnIde<Action> SuiteId --projects P... --args ARGS...      (selected projects)
:: --   FnIde<Action> SuiteId --args ARGS... FLAGS...             (all projects)
:: --
...
:Missing
    CALL FnEtcLogRun FnIdeDispatch "fn=Fn%Function_Pascal%%Function_Suffix% suite=%Function_SuiteId% project=%Function_ProjectId% args=%Function_Args% projects=%Function_Projects% flags=%Function_Passthru%"
    IF DEFINED Function_DryRun GOTO Destructor
    SET "Function_Error=Not implemented yet: Fn%Function_Pascal%%Function_Suffix%"
    GOTO Failure

:Constructor
    SET "Function_Module=ide"
    SET "Function_Pascal=Ide"
    :: <action>:<function suffix>
    SET "Function_Actions=code:Code jetbrains:Jetbrains rider:Rider webstorm:Webstorm"
...
:Usage
    CALL FnEtcLogError FnIdeDispatch "%Function_Error%"
    SET "Function_Error="
    ECHO   Usage: ^<SUITE^> ide ^<ACTION^> ^<ARGS...^> ^<FLAGS...^>
    ECHO          ^<SUITE^> ^<PROJECT^> ide ^<ACTION^> ^<ARGS...^>
    ECHO          ^<SUITE^> ide ^<ACTION^> -p ^<PROJECTS...^>
    ECHO   Actions: code jetbrains rider webstorm
    GOTO Failure
...
:Destructor
    ...
    IF DEFINED Function_Error CALL FnEtcLogError FnIdeDispatch "%Function_Error%"
