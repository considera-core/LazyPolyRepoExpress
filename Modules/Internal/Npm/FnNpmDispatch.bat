...
:Missing
    CALL FnEtcLogRun FnNpmDispatch "fn=Fn%Input_Pascal%%Input_Suffix% suite=%Input_SuiteId% project=%Input_ProjectId% args=%Input_Args% projects=%Input_Projects% flags=%Input_Passthru%"
    IF DEFINED Input_DryRun GOTO Destructor
    SET "Input_Error=Not implemented yet: Fn%Input_Pascal%%Input_Suffix%"
    GOTO Failure

:Constructor
    SET "Input_Module=npm"
    SET "Input_Pascal=Npm"
    :: <action>:<function suffix>
    SET "Input_Actions=install:Install run:Run build:Build test:Test"
...
:Usage
    CALL FnEtcLogError FnNpmDispatch "%Input_Error%"
    SET "Input_Error="
...
:Destructor
    ...
    IF DEFINED Input_Error CALL FnEtcLogError FnNpmDispatch "%Input_Error%"
