:: FnNpmDispatch <SuiteId> <ProjectId> <ActionId> <Arg[]> <Flag[]>
:: leprechaun function npm dispatch <SuiteId> <ProjectId> <ActionId> <Arg[]> <Flag[]>
:: -- Validates the action for the npm module and invokes the leaf function:
:: --
:: --   FnNpm<Action> SuiteId ProjectId --args ARGS... FLAGS...   (single project)
:: --   FnNpm<Action> SuiteId --projects P... --args ARGS...      (selected projects)
:: --   FnNpm<Action> SuiteId --args ARGS... FLAGS...             (all projects)
:: --
...
:Missing
    CALL FnEtcLogRun FnNpmDispatch "fn=Fn%Function_Pascal%%Function_Suffix% suite=%Function_SuiteId% project=%Function_ProjectId% args=%Function_Args% projects=%Function_Projects% flags=%Function_Passthru%"
    IF DEFINED Function_DryRun GOTO Destructor
    SET "Function_Error=Not implemented yet: Fn%Function_Pascal%%Function_Suffix%"
    GOTO Failure

:Constructor
    SET "Function_Module=npm"
    SET "Function_Pascal=Npm"
    :: <action>:<function suffix>
    SET "Function_Actions=install:Install run:Run build:Build test:Test"
...
:Usage
    CALL FnEtcLogError FnNpmDispatch "%Function_Error%"
    SET "Function_Error="
...
:Destructor
    ...
    IF DEFINED Function_Error CALL FnEtcLogError FnNpmDispatch "%Function_Error%"
