:: FnGitDispatch --suite <SUITE> --action <ACTION> [--project <PROJECT>]
::               [-p <PROJECTS...>] [-a <ARGS...>] [FLAGS...]
:: -- Validates the action for the git module and invokes the leaf function
:: --
:: --   FnGit<Action> SUITE PROJECT ARGS... FLAGS...          (single project)
:: --   FnGit<Action> SUITE ARGS... -p PROJECTS... FLAGS...   (selected projects)
:: --   FnGit<Action> SUITE ARGS... FLAGS...                  (all projects)
:: --
...
SET "M_ACTIONS=branch:Branch branches:Branches pull:Pull home:Home story:Story refresh-dependabot:RefreshDependabot"
...
:MISSING_SUITE
    ECHO LeprechaunCLI:FnGitDispatch[E]: Missing required flag --suite ^<SUITE^>
    GOTO :USAGE

:MISSING_ACTION
    ECHO LeprechaunCLI:FnGitDispatch[E]: Missing action
    GOTO :USAGE

:UNKNOWN_ACTION
    ECHO LeprechaunCLI:FnGitDispatch[E]: Unknown git action "%M_ACTION%"
    GOTO :USAGE
...
:USAGE
    ...
    ECHO   Actions: branch branches pull home story refresh-dependabot
    EXIT /B 1
