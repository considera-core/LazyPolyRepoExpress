:: FnAppDispatch --suite <SUITE> --action <ACTION> [--project <PROJECT>]
::               [-p <PROJECTS...>] [-a <ARGS...>] [FLAGS...]
:: -- Validates the action for the app module and invokes the leaf function
:: --
:: --   FnApp<Action> SUITE PROJECT ARGS... FLAGS...          (single project)
:: --   FnApp<Action> SUITE ARGS... -p PROJECTS... FLAGS...   (selected projects)
:: --   FnApp<Action> SUITE ARGS... FLAGS...                  (all projects)
:: --
...
SET "M_ACTIONS=run:Run launch:Launch validate:Validate list:List"
...
:MISSING_SUITE
    ECHO LeprechaunCLI:FnAppDispatch[E]: Missing required flag --suite ^<SUITE^>
    GOTO :USAGE

:MISSING_ACTION
    ECHO LeprechaunCLI:FnAppDispatch[E]: Missing action
    GOTO :USAGE

:UNKNOWN_ACTION
    ECHO LeprechaunCLI:FnAppDispatch[E]: Unknown app action "%M_ACTION%"
    GOTO :USAGE

:USAGE
    ECHO   Usage: ^<SUITE^> app ^<ACTION^> ^<ARGS...^> ^<FLAGS...^>
    ECHO          ^<SUITE^> ^<PROJECT^> app ^<ACTION^> ^<ARGS...^>
    ECHO          ^<SUITE^> app ^<ACTION^> -p ^<PROJECTS...^>
    ECHO   Actions: run launch validate list
    EXIT /B 1
