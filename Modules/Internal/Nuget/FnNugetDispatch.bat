:: FnNugetDispatch <SuiteId> <ProjectId> <ActionId> <Arg[]> <Flag[]>
:: leprechaun function nuget dispatch <SuiteId> <ProjectId> <ActionId> <Arg[]> <Flag[]>
:: -- Validates the action for the nuget module and invokes the leaf function:
:: --
:: --   FnNuget<Action> SuiteId ProjectId --args ARGS... FLAGS...   (single project)
:: --   FnNuget<Action> SuiteId --projects P... --args ARGS...      (selected projects)
:: --   FnNuget<Action> SuiteId --args ARGS... FLAGS...             (all projects)
:: --
...
:Missing
    CALL FnEtcLogRun FnNugetDispatch "fn=Fn%Function_Pascal%%Function_Suffix% suite=%Function_SuiteId% project=%Function_ProjectId% args=%Function_Args% projects=%Function_Projects% flags=%Function_Passthru%"
    IF DEFINED Function_DryRun GOTO Destructor
    SET "Function_Error=Not implemented yet: Fn%Function_Pascal%%Function_Suffix%"
    GOTO Failure

:Constructor
    SET "Function_Module=nuget"
    SET "Function_Pascal=Nuget"
    :: <action>:<function suffix>
    SET "Function_Actions=restore:Restore pack:Pack push:Push"
...
:Usage
    CALL FnEtcLogError FnNugetDispatch "%Function_Error%"
    SET "Function_Error="
    ECHO   Usage: ^<SUITE^> nuget ^<ACTION^> ^<ARGS...^> ^<FLAGS...^>
    ECHO          ^<SUITE^> ^<PROJECT^> nuget ^<ACTION^> ^<ARGS...^>
    ECHO          ^<SUITE^> nuget ^<ACTION^> -p ^<PROJECTS...^>
    ECHO   Actions: restore pack push
    GOTO Failure
...
:Destructor
    ...
    IF DEFINED Function_Error CALL FnEtcLogError FnNugetDispatch "%Function_Error%"
