:: FnAwsDispatch --suite <SUITE> --action <ACTION> [--project <PROJECT>]
::               [-p <PROJECTS...>] [-a <ARGS...>] [FLAGS...]
:: -- Validates the action for the aws module and invokes the leaf function
:: --
:: --   FnAws<Action> SUITE PROJECT ARGS... FLAGS...          (single project)
:: --   FnAws<Action> SUITE ARGS... -p PROJECTS... FLAGS...   (selected projects)
:: --   FnAws<Action> SUITE ARGS... FLAGS...                  (all projects)
:: --
...
SET "M_ACTIONS=login:Login whoami:Whoami env:Env"
...
:MISSING_SUITE
    ECHO LeprechaunCLI:FnAwsDispatch[E]: Missing required flag --suite ^<SUITE^>
    GOTO :USAGE

:MISSING_ACTION
    ECHO LeprechaunCLI:FnAwsDispatch[E]: Missing action
    GOTO :USAGE

:UNKNOWN_ACTION
    ECHO LeprechaunCLI:FnAwsDispatch[E]: Unknown aws action "%M_ACTION%"
    GOTO :USAGE
