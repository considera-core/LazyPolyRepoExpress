:: FnAwsDispatch <SuiteId> <ProjectId> <ActionId> <Arg[]> <Flag[]>
:: leprechaun function aws dispatch <SuiteId> <ProjectId> <ActionId> <Arg[]> <Flag[]>
:: -- Validates the action for the aws module and invokes the leaf function:
:: --
:: --   FnAws<Action> SuiteId ProjectId --args ARGS... FLAGS...   (single project)
:: --   FnAws<Action> SuiteId --projects P... --args ARGS...      (selected projects)
:: --   FnAws<Action> SuiteId --args ARGS... FLAGS...             (all projects)
:: --
...
:Missing
    CALL FnEtcLogRun FnAwsDispatch "fn=Fn%Function_Pascal%%Function_Suffix% suite=%Function_SuiteId% project=%Function_ProjectId% args=%Function_Args% projects=%Function_Projects% flags=%Function_Passthru%"
    IF DEFINED Function_DryRun GOTO Destructor
    SET "Function_Error=Not implemented yet: Fn%Function_Pascal%%Function_Suffix%"
    GOTO Failure

:Constructor
    SET "Function_Module=aws"
    SET "Function_Pascal=Aws"
    :: <action>:<function suffix>
    SET "Function_Actions=login:Login whoami:Whoami env:Env"
...
:Usage
    CALL FnEtcLogError FnAwsDispatch "%Function_Error%"
    SET "Function_Error="
    ECHO   Usage: ^<SUITE^> aws ^<ACTION^> ^<ARGS...^> ^<FLAGS...^>
    ECHO          ^<SUITE^> ^<PROJECT^> aws ^<ACTION^> ^<ARGS...^>
    ECHO          ^<SUITE^> aws ^<ACTION^> -p ^<PROJECTS...^>
    ECHO   Actions: login whoami env
    GOTO Failure
...
:Destructor
    ...
    IF DEFINED Function_Error CALL FnEtcLogError FnAwsDispatch "%Function_Error%"
