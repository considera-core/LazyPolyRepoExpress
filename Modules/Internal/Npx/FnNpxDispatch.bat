:: FnNpxDispatch <SuiteId> <ProjectId> <ActionId> <Arg[]> <Flag[]>
:: leprechaun function npx dispatch <SuiteId> <ProjectId> <ActionId> <Arg[]> <Flag[]>
:: -- Validates the action for the npx module and invokes the leaf function:
:: --
:: --   FnNpx<Action> SuiteId ProjectId --args ARGS... FLAGS...   (single project)
:: --   FnNpx<Action> SuiteId --projects P... --args ARGS...      (selected projects)
:: --   FnNpx<Action> SuiteId --args ARGS... FLAGS...             (all projects)
:: --
...
:Missing
    CALL FnEtcLogRun FnNpxDispatch "fn=Fn%Function_Pascal%%Function_Suffix% suite=%Function_SuiteId% project=%Function_ProjectId% args=%Function_Args% projects=%Function_Projects% flags=%Function_Passthru%"
    IF DEFINED Function_DryRun GOTO Destructor
    SET "Function_Error=Not implemented yet: Fn%Function_Pascal%%Function_Suffix%"
    GOTO Failure

:Constructor
    SET "Function_Module=npx"
    SET "Function_Pascal=Npx"
    :: <action>:<function suffix>
    SET "Function_Actions=run:Run"
...
:Usage
    CALL FnEtcLogError FnNpxDispatch "%Function_Error%"
    SET "Function_Error="
    ECHO   Usage: ^<SUITE^> npx ^<ACTION^> ^<ARGS...^> ^<FLAGS...^>
    ECHO          ^<SUITE^> ^<PROJECT^> npx ^<ACTION^> ^<ARGS...^>
    ECHO          ^<SUITE^> npx ^<ACTION^> -p ^<PROJECTS...^>
    ECHO   Actions: run
    GOTO Failure
...
:Destructor
    ...
    IF DEFINED Function_Error CALL FnEtcLogError FnNpxDispatch "%Function_Error%"
