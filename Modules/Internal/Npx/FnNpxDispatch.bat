...
:Missing
    CALL FnEtcLogRun FnNpxDispatch "fn=Fn%Input_Pascal%%Input_Suffix% suite=%Input_SuiteId% project=%Input_ProjectId% args=%Input_Args% projects=%Input_Projects% flags=%Input_Passthru%"
    IF DEFINED Input_DryRun GOTO Destructor
    SET "Input_Error=Not implemented yet: Fn%Input_Pascal%%Input_Suffix%"
    GOTO Failure

:Constructor
    SET "Input_Module=npx"
    SET "Input_Pascal=Npx"
    :: <action>:<function suffix>
    SET "Input_Actions=run:Run"
...
:Usage
    CALL FnEtcLogError FnNpxDispatch "%Input_Error%"
    SET "Input_Error="
...
:Destructor
    ...
    IF DEFINED Input_Error CALL FnEtcLogError FnNpxDispatch "%Input_Error%"
