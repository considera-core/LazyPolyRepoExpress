:: FnTestDispatch --suite <SUITE> --action <ACTION> [--project <PROJECT>]
::               [-p <PROJECTS...>] [-a <ARGS...>] [FLAGS...]
:: -- Validates the action for the test module and invokes the leaf function
:: --
:: --   FnTest<Action> SUITE PROJECT ARGS... FLAGS...          (single project)
:: --   FnTest<Action> SUITE ARGS... -p PROJECTS... FLAGS...   (selected projects)
:: --   FnTest<Action> SUITE ARGS... FLAGS...                  (all projects)
:: --
...
SET "M_ACTIONS=list:List run:Run"
...
:MISSING_SUITE
    ECHO LeprechaunCLI:FnTestDispatch[E]: Missing required flag --suite ^<SUITE^>
    GOTO :USAGE

:MISSING_ACTION
    ECHO LeprechaunCLI:FnTestDispatch[E]: Missing action
    GOTO :USAGE

:UNKNOWN_ACTION
    ECHO LeprechaunCLI:FnTestDispatch[E]: Unknown test action "%M_ACTION%"
    GOTO :USAGE
