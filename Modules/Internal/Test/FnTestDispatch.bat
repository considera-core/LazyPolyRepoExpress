:: FnTestDispatch <SuiteId> <ProjectId> <ActionId> <Arg[]> <Flag[]>
:: leprechaun function test dispatch <SuiteId> <ProjectId> <ActionId> <Arg[]> <Flag[]>
:: -- Validates the action for the test module and invokes the leaf function:
:: --
:: --   FnTest<Action> SuiteId ProjectId --args ARGS... FLAGS...   (single project)
:: --   FnTest<Action> SuiteId --projects P... --args ARGS...      (selected projects)
:: --   FnTest<Action> SuiteId --args ARGS... FLAGS...             (all projects)
:: --
...
:Missing
    CALL FnEtcLogRun FnTestDispatch "fn=Fn%Function_Pascal%%Function_Suffix% suite=%Function_SuiteId% project=%Function_ProjectId% args=%Function_Args% projects=%Function_Projects% flags=%Function_Passthru%"
    IF DEFINED Function_DryRun GOTO Destructor
    SET "Function_Error=Not implemented yet: Fn%Function_Pascal%%Function_Suffix%"
    GOTO Failure

:Constructor
    SET "Function_Module=test"
    SET "Function_Pascal=Test"
    :: <action>:<function suffix>
    SET "Function_Actions=list:List run:Run"
...
:Usage
    CALL FnEtcLogError FnTestDispatch "%Function_Error%"
    SET "Function_Error="
    ECHO   Usage: ^<SUITE^> test ^<ACTION^> ^<ARGS...^> ^<FLAGS...^>
    ECHO          ^<SUITE^> ^<PROJECT^> test ^<ACTION^> ^<ARGS...^>
    ECHO          ^<SUITE^> test ^<ACTION^> -p ^<PROJECTS...^>
    ECHO   Actions: list run
    GOTO Failure
...
:Destructor
    ...
    IF DEFINED Function_Error CALL FnEtcLogError FnTestDispatch "%Function_Error%"
