:: FnUtilityDispatch <SuiteId> <ProjectId> <ActionId> <Arg[]> <Flag[]>
:: leprechaun function utility dispatch <SuiteId> <ProjectId> <ActionId> <Arg[]> <Flag[]>
:: -- Validates the action for the utility module and invokes the leaf function:
:: --
:: --   FnUtility<Action> SuiteId ProjectId --args ARGS... FLAGS...   (single project)
:: --   FnUtility<Action> SuiteId --projects P... --args ARGS...      (selected projects)
:: --   FnUtility<Action> SuiteId --args ARGS... FLAGS...             (all projects)
:: --
...
:Missing
    CALL FnEtcLogRun FnUtilityDispatch "fn=Fn%Function_Pascal%%Function_Suffix% suite=%Function_SuiteId% project=%Function_ProjectId% args=%Function_Args% projects=%Function_Projects% flags=%Function_Passthru%"
    IF DEFINED Function_DryRun GOTO Destructor
    SET "Function_Error=Not implemented yet: Fn%Function_Pascal%%Function_Suffix%"
    GOTO Failure

:Constructor
    SET "Function_Module=utility"
    SET "Function_Pascal=Utility"
    :: <action>:<function suffix>
    SET "Function_Actions=tree:Tree path:Path list:List"
...
:Usage
    CALL FnEtcLogError FnUtilityDispatch "%Function_Error%"
    SET "Function_Error="
    ECHO   Usage: ^<SUITE^> utility ^<ACTION^> ^<ARGS...^> ^<FLAGS...^>
    ECHO          ^<SUITE^> ^<PROJECT^> utility ^<ACTION^> ^<ARGS...^>
    ECHO          ^<SUITE^> utility ^<ACTION^> -p ^<PROJECTS...^>
    ECHO   Actions: tree path list
    GOTO Failure
...
:Destructor
    ...
    IF DEFINED Function_Error CALL FnEtcLogError FnUtilityDispatch "%Function_Error%"
