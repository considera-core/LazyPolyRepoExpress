...
:Missing
    CALL FnEtcLogRun FnUtilityDispatch "fn=Fn%Input_Pascal%%Input_Suffix% suite=%Input_SuiteId% project=%Input_ProjectId% args=%Input_Args% projects=%Input_Projects% flags=%Input_Passthru%"
    IF DEFINED Input_DryRun GOTO Destructor
    SET "Input_Error=Not implemented yet: Fn%Input_Pascal%%Input_Suffix%"
    GOTO Failure

:Constructor
    SET "Input_Module=utility"
    SET "Input_Pascal=Utility"
    :: <action>:<function suffix>
    SET "Input_Actions=tree:Tree path:Path list:List"
...
:Usage
    CALL FnEtcLogError FnUtilityDispatch "%Input_Error%"
    SET "Input_Error="
...
:Destructor
    ...
    IF DEFINED Input_Error CALL FnEtcLogError FnUtilityDispatch "%Input_Error%"
