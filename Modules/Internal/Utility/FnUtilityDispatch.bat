:: FnUtilityDispatch --suite <SUITE> --action <ACTION> [--project <PROJECT>]
::               [-p <PROJECTS...>] [-a <ARGS...>] [FLAGS...]
:: -- Validates the action for the utility module and invokes the leaf function
:: --
:: --   FnUtility<Action> SUITE PROJECT ARGS... FLAGS...          (single project)
:: --   FnUtility<Action> SUITE ARGS... -p PROJECTS... FLAGS...   (selected projects)
:: --   FnUtility<Action> SUITE ARGS... FLAGS...                  (all projects)
:: --
...
SET "M_MODULE=Utility"
SET "M_LABEL=utility"
:: <action>:<function suffix>
SET "M_ACTIONS=tree:Tree path:Path list:List"
...
:MISSING_SUITE
    ECHO LeprechaunCLI:FnUtilityDispatch[E]: Missing required flag --suite ^<SUITE^>
    GOTO :USAGE

:MISSING_ACTION
    ECHO LeprechaunCLI:FnUtilityDispatch[E]: Missing action
    GOTO :USAGE

:UNKNOWN_ACTION
    ECHO LeprechaunCLI:FnUtilityDispatch[E]: Unknown utility action "%M_ACTION%"
    GOTO :USAGE
